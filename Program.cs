...
// Definicion e inicilizacion de los arreglos en paralelo  diccionario
...
Dictionary<string, int> diccionario = Departamento.Zip(Poblacion, (k, v) => new { Clave = k, Valor = v }).ToDictionary(x => x.Clave, x => x.Valor);
...
Console.WriteLine($"Dpto. con mayor Población:{maxDepKey}");
Console.WriteLine($"Dpto. con menor Población:{minDepKey}");